...
# todo - make a class edge so I can use heapq library - can import heap from scratch, but doing it this way for now
...
def calculate_distances(adjacency_list):
    ...
    for i in range(len(adjacency_list)):
        ...
        while len(current) > 0:
            ...
            if min_edge.end_node not in memo_distances[i]:
                memo_distances[i][min_edge.end_node] = min_edge.distance
            elif min_edge.distance < memo_distances[i][min_edge.end_node]:
                memo_distances[i][min_edge.end_node] = min_edge.distance
...
class Saving:
    ...
    def add_merged_route(self, routes):
        # check if both targets are either beginning or ending points of the route
        route_1 = routes[self.target_1.route]
        pos_in_route = route_1.index(self.target_1)
        if pos_in_route == 0:
            route_1.reverse()
        
        # same for target 2
        route_2 = routes[self.target_2.route]
...
def calculate_cost(adjacency_list, targets, vehicle, recalculate_distances = False):
    ...
    while len(routes) > 1 and len(savings) > 1:
        ...
        route_1 = routes[top_saving.target_1.route]
        route_2 = routes[top_saving.target_2.route]
